...
def make_embedding_requested_handler(services_provider: Callable[[], Any]):
    def _handle(payload: dict[str, Any]) -> None:
        ...
        if not training_item_id or not knowledge_base_id or not understanding_job_id or not discovery_job_id:
            logger.error("kb.embedding_requested: érvénytelen payload: %r", payload)
            return
        ...
        try:
            ...
            try:
                ...
                if exc.code == EmbeddingErrorCode.JOB_ALREADY_RUNNING.value:
                    logger.info(
                        "kb.embedding_requested: már fut embedding job (discovery=%s)",
                        discovery_job_id,
                    )
                    return
                raise
            logger.info(
                "kb.embedding_requested feldolgozva (item=%s discovery=%s status=%s)",
                training_item_id,
                discovery_job_id,
                status.value,
            )
...
__all__ = ["make_embedding_requested_handler"]
